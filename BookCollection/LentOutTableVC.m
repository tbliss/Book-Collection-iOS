...
@interface LentOutTableVC ()

@end

@implementation LentOutTableVC
...
- (void) refreshTable
{
    ...
    self.books = self.bookCollection.lentBooks;
...
{
    static NSString *CellIdentifier = @"WishlistBook";
    ...
    if (indexPath.section == 0) {
        for (int i = 0; i < self.bookCount; i++) {
            if (indexPath.row == i) {
                //NSDictionary *currBook = [self.books objectAtIndex:i];
                //cell.textLabel.text = [currBook objectForKey:@"title"];
...
{
    if ([@"WishlistBookView" isEqualToString:segue.identifier]) {
    ...
    }  else if ([@"AddBook3" isEqualToString:segue.identifier]) {
